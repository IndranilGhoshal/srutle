...
import SellerList from './_list/SellerList'
import { useRouter } from 'next/navigation';
import { sellerApi } from '../lib/apiService';
import { toast, ToastContainer } from 'react-toastify';

export default function SellerManagementComponent() {
  ...
  useEffect(() => {
    if (!getLocalStorageData("admin")) {
      ...
      router.push("/admin/account/sellermanagement");
  ...
  const getData = async (r, l, s, sta, n, e, c) => {
    ...
    let response = await sellerApi(data)
    if (response.success) {
      hideLoader()
  ...
  const onStatus = async (val, id) => {
    let response = await sellerApi({ status: val, id: id, onStatus: true })
    if (response.success) {
    ...
    <>
      <div className="main-das-page">
        <div className="heading my-3">
          <h3 className='text-blue'>Seller Management</h3>
        </div>
        <div className='mt-2'>
          <h5 className='no-of-count'>No. of Total Result Count : {listlength}</h5>
        </div>
