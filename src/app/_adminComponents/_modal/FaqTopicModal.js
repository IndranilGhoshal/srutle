'use client'
import { adminFaqApi, uploadImageApi } from '@/app/lib/apiService';
...
export default function FaqTopicModal({ onMessage, isedit }) {
    ...
    const addEvent = async () => {
        ...
        if (err == 0) {
            showLoader()
            let data = { image, title, description, status, addfaqtopic: true }
                ...
                <Modal.Body style={{ padding: 0 }}>
                    <div className="modal-content">
                        <div className="modal-header border-0">
                            <h4 className='text-blue'>Add FAQ Topic</h4>
                            <button
                                id="clsBtn12"
                                onClick={() => { handleClose() }}
                                type="button"
