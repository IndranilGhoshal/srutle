...
import { categorySchema, productSchema, productTypesAttributeSchema, productTypesAttributeValueSchema, productTypesSchema, subCategorySchema } from "@/app/model/adminModel";
...
export async function POST(request) {
    ...
    if (middeleware) {
        try {
            ...
            if (payload.list) {
                ...
                if (results.length > 0) {
                    ...
                    for (let res of r) {
                        ...
                        let noOfItems = await productSchema.find({ mstproducttypeid: res._id, status: { $in: ['0', '1'] } });
            ...
            return NextResponse.json({ result, success, message, listlength, producttype, subcategory, category , status: responsestatus, error: 0 })
        } catch (e) {
