...
import { giftcardamountsSchema, giftcarddetailsSchema, giftcardimagesSchema, giftcardsSchema, roleSchema } from "@/app/model/adminModel";
            ...
            if (payload.list) {
                ...
                if (results.length > 0) {
                    ...
                    for (let r of results) {
                        let noofuse = await giftcarddetailsSchema.find({mstgiftcardsid:r._id, status: { $in: ['0'] }})
                        let data = {
                            "_id": r._id,
                            "image": r.image,
                            "title": r.title,
                            "description": r.description,
                            noofuse:noofuse.length,
                            "status": r.status,
