...
import { consumerSchema } from "@/app/model/adminModel";
    ...
    if (middeleware) {
        try {
            ...
            if (payload.list) {
                ...
                // ------------List Length Start------------ //
                let len = await consumerSchema.find(filter)
                listlength = len.length
                // ------------List Length End------------ //

                // ------------List Start------------ //
                let result1 = await consumerSchema.find(filter).sort(sortObj).limit(payload.limit).skip(payload.skip);
                if (result1.length > 0) {
            ...
            //Status
                ...
                result = await consumerSchema.findOneAndUpdate({ _id: payload.id }, { status: payload.status })
                if (result) {
                    if (payload.status == "2") {
                        message = "Consumer delete successfully"
                    } else if (payload.status == "1") {
                        message = "Consumer inactive successfully"
                    } else {
                        message = "Consumer active successfully"
                    }
                    responsestatus = StatusCodes.SUCCESS
                    success = true
                } else {
