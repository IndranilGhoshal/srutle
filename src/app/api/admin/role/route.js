        ...
        try {
            ...
            if (payload.list) {
                ...
                if (results.length > 0) {
                    ...
                    let temp = [];
                    for (let res of r) {
                        let nouser = await adminSchema.find({ mstroleid: res._id, status: { $in: ['0', '1', '3'] } });
                        let data = {
                            "_id": res._id,
                            "role": res.role,
            ...
            //Status -> Active, Inactive, Delete
                ...
                let nouser = await adminSchema.find({ mstroleid: payload.id, status: { $in: ['0', '3'] } });
                if (payload.status == "1" && nouser.length == 0) {
                    let upresult = await roleSchema.findOneAndUpdate({ _id: payload.id }, { status: payload.status })
                    if (upresult) {
                        responsestatus = StatusCodes.SUCCESS
                        success = true
                        result = null
                        message = upresult.role + " Role inactive successfully"
                    } else {
                        responsestatus = StatusCodes.INTERNAL_SERVER_ERROR
                        success = false
                        result = null
                ...
                else if (payload.status == "1" && nouser.length > 0) {
                    responsestatus = StatusCodes.FOUND
                    success = false
                    result = null
                    message = "Role did not inactive. Role have active user."
                }
                else if (payload.status == "2" && nouser.length == 0) {
                    let upresult = await roleSchema.findOneAndUpdate({ _id: payload.id }, { status: payload.status })
                    if (upresult) {
                        responsestatus = StatusCodes.SUCCESS
                        success = true
                        result = null
                        message = upresult.role + " Role delete successfully"
                    } else {
                        responsestatus = StatusCodes.INTERNAL_SERVER_ERROR
                        success = false
                        result = null
                    ...
                    success = false
                    result = null
                    message = "Role did not delete. Role have active user."
                }
                    ...
                    if (upresult) {
                        ...
                        success = true
                        result = null
                        message = upresult.role + " Role active successfully"
                    } else {
                        responsestatus = StatusCodes.INTERNAL_SERVER_ERROR
                        success = false
                        result = null
